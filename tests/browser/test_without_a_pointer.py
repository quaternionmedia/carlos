...
class TestTabIsFocus:
    ...
    def test_tab_walks_the_controls(self, page):
        classes = " ".join(focus_ring(page, 12))
...
class TestAKnobAnswersToKeys:
    ...
    def test_arrows_move_it(self, page, key, direction):
        knob = page.locator(".knob").first
        knob.focus()
    ...
    def test_shift_is_finer_than_a_bare_press(self, page):
        knob = page.locator(".knob").first
        knob.focus()
    ...
    def test_home_and_end_reach_the_stops(self, page):
        knob = page.locator(".knob").first
        knob.focus()
    ...
    def test_it_announces_the_value_it_is_showing(self, page):
...
class TestPatchingWithoutAPointer:
    def test_enter_on_two_sockets_makes_a_cable(self, page):
        ...
        assert "armed" in page.locator("#status").inner_text()
    ...
    def test_space_works_the_same_way(self, page):
        page.locator('.face.active .jack[data-type="output"]').first.focus()
        page.keyboard.press(" ")
        assert "armed" in page.locator("#status").inner_text()

    def test_escape_lets_go_of_the_selection_and_the_focus(self, page):
        page.locator(".module").first.click()
        assert page.locator(".module.selected").count() == 1

        page.keyboard.press("Escape")
        assert page.locator(".module.selected").count() == 0
